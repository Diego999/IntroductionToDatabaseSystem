...
class Milestone3querykAsynchronous extends ILARIA_ApplicationAsynchronous
{
    protected function getUniqueIdentifier()
    {
        return "milestone3queryk";
    }

    protected function getWebPath($params)
    {
        return ILARIA_ConfigurationGlobal::buildRequestChain("milestone3", "queryk", $params);
    }

    protected function getDisplayStructure($params)
    {
        return "<table class=\"table\" id=\"" . $this->getContainerId() . "\">"
            . "<tr><th>Year</th><th>Company</th><th>Number of movies</th></tr>"
    ...
    {
        return "<div>"
        . "<h3 class=\"panel-title\">Three companies that released most movies per year</h3></div>"
    ...
    protected function getRawContent($params)
    {
        ...
        $content = $model->queryK();
        if (!is_array($content))
        {
            throw new ILARIA_CoreError("An error occurred while executing query K",
