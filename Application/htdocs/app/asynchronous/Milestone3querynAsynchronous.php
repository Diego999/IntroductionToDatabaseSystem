...
class Milestone3querynAsynchronous extends ILARIA_ApplicationAsynchronous
{
    protected function getUniqueIdentifier()
    {
        return "milestone3queryn";
    }

    protected function getWebPath($params)
    {
        return ILARIA_ConfigurationGlobal::buildRequestChain("milestone3", "queryn", $params);
    }

    protected function getDisplayStructure($params)
    {
        return "<table class=\"table\" id=\"" . $this->getContainerId() . "\">"
            . "<tr><th>Country ID</th><th>Character ID</th></tr>"
    ...
    {
        return "<div>"
        . "<h3 class=\"panel-title\">Most frequent character name from production companies for each country</h3></div>"
    ...
    protected function getRawContent($params)
    {
        ...
        $content = $model->queryN();
        if (!is_array($content))
        {
            throw new ILARIA_CoreError("An error occurred while executing query N",
