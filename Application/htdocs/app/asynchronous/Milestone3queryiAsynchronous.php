...
class Milestone3queryiAsynchronous extends ILARIA_ApplicationAsynchronous
{
    protected function getUniqueIdentifier()
    {
        return "milestone3queryi";
    }

    protected function getWebPath($params)
    {
        return ILARIA_ConfigurationGlobal::buildRequestChain("milestone3", "queryi", $params);
    }

    protected function getDisplayStructure($params)
    {
        return "<table class=\"table\" id=\"" . $this->getContainerId() . "\">"
            . "<tr><th>Serie</th><th>Number</th></tr>"
    ...
    {
        return "<div>"
        . "<h3 class=\"panel-title\">Top-ten tv-series (by number of episodes per season)</h3></div>"
    ...
    protected function getRawContent($params)
    {
        ...
        $content = $model->queryI();
        if (!is_array($content))
        {
            throw new ILARIA_CoreError("An error occurred while executing query I",
