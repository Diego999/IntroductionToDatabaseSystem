...
class Milestone3querydAsynchronous extends ILARIA_ApplicationAsynchronous
{
    protected function getUniqueIdentifier()
    {
        return "milestone3queryd";
    }

    protected function getWebPath($params)
    {
        return ILARIA_ConfigurationGlobal::buildRequestChain("milestone3", "queryd", $params);
    }

    protected function getDisplayStructure($params)
    {
        return "<table class=\"table\" id=\"" . $this->getContainerId() . "\">"
            . "<tr><th>Person</th><th>Production</th></tr>"
    ...
    {
        return "<div>"
        . "<h3 class=\"panel-title\">Persons who worked with spouses/children/relatives on same productions</h3></div>"
    ...
    protected function getRawContent($params)
    {
        ...
        $content = $model->queryD();
        if (!is_array($content))
        {
            throw new ILARIA_CoreError("An error occurred while executing query D",
