...
class Milestone3queryjAsynchronous extends ILARIA_ApplicationAsynchronous
{
    protected function getUniqueIdentifier()
    {
        return "milestone3queryj";
    }

    protected function getWebPath($params)
    {
        return ILARIA_ConfigurationGlobal::buildRequestChain("milestone3", "queryj", $params);
    }

    protected function getDisplayStructure($params)
    {
        return "<table class=\"table\" id=\"" . $this->getContainerId() . "\">"
            . "<tr><th>Person ID</th></tr>"
    ...
    {
        return "<div>"
        . "<h3 class=\"panel-title\">Actors, actresses and directors having movies released after their death</h3></div>"
    ...
    protected function getRawContent($params)
    {
        ...
        $content = $model->queryJ();
        if (!is_array($content))
        {
            throw new ILARIA_CoreError("An error occurred while executing query J",
