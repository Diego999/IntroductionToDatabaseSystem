...
class KindController extends ILARIA_ApplicationController
{
    ...
    {
        // Instanciate model
        $model = $this->getModel("kind");

        // Instanciate view
        $view = $this->getView("kindform");
        ...
        if ($request->existPostArg("f_" . self::FORM_NAME . "_dataid"))
        {
            ...
            if ($model->insert($name) == 0)
            {
                ...
                $view->addAlert(ILARIA_ApplicationView::ALERT_TYPE_SUCCESS, "The kind \"" . $name . "\" was successfully inserted");
                return $view;
            }
            else
            {
                // Register error
                $view->addAlert(ILARIA_ApplicationView::ALERT_TYPE_DANGER, "The kind \"" . $name . "\" was not inserted");
            }
        }
    ...
    public function action_update($request)
    {
        ...
        if ($request->existPostArg("f_" . self::FORM_NAME . "_dataid"))
        {
            ...
            if ($model->update($id, $name) == 0)
            {
                ...
                $controller = ILARIA_CoreLoader::getInstance()->loadController("directaccess");
                $view = $controller->action_miscellaneous(NULL);
                $view->addAlert(ILARIA_ApplicationView::ALERT_TYPE_SUCCESS, "The kind \"" . $name . "\" was successfully updated");
                return $view;
            }
            else
            {
                // Register error
                $view->addAlert(ILARIA_ApplicationView::ALERT_TYPE_DANGER, "The kind \"" . $name . "\" was not updated");
            }
        }
    ...
    public function action_delete($request)
    {
        ...
        if ($request->existGetArg("confirm"))
        {
            ...
            {
                $view->addAlert(ILARIA_ApplicationView::ALERT_TYPE_SUCCESS, "The kind \"" . $infos['name'] . "\" was successfully deleted");
            }
            else
            {
                $view->addAlert(ILARIA_ApplicationView::ALERT_TYPE_DANGER, "The kind \"" . $infos['name'] . "\" failed to be deleted");
            }
        ...
        {
            // Create view
            $view = "<p>The kind \\\"" . $infos['name'] . "\\\" will be deleted. Are you sure ?</p>";
            ...
            return ILARIA_ApplicationAsynchronous::buildModalAjaxResponse(array(
                ...
                ILARIA_ApplicationAsynchronous::MODAL_BUTTONS => array(
                    ...
                    array(
                        ...
                        ILARIA_ApplicationAsynchronous::MODAL_BUTTON_LINK => ILARIA_ConfigurationGlobal::buildRequestChain("kind", "delete", array(
                            'id' => $id,
                            'confirm' => 'confirm',
                        )),
                    ),
