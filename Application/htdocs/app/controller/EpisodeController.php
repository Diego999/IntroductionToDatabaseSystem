...
class EpisodeController extends ILARIA_ApplicationController
{
    const FORM_NAME = 'episode';
    ...
    {
        // Instanciate model
        $model = $this->getModel("episode");

        // Instanciate view
        $view = $this->getView("episodeform");
        ...
        // Gather serie ID
        $serieId = $request->getGetArg('serieid');
        ...
        if ($request->existPostArg("f_" . self::FORM_NAME . "_dataid"))
        {
            ...
            $seasonNumber = ILARIA_SecurityManager::in($request->getPostArg("f_" . self::FORM_NAME . "_season_number"));
            $episodeNumber = ILARIA_SecurityManager::in($request->getPostArg("f_" . self::FORM_NAME . "_episode_number"));

            // Insert values
            if (($id = $model->insert($title, $year, $episodeNumber, $seasonNumber, $serieId)) >= 0)
            {
                ...
                $view->addAlert(ILARIA_ApplicationView::ALERT_TYPE_SUCCESS, "The episode \"" . $title . "\" was successfully inserted");
                return $view;
            }
            else
            {
                // Register error
                $view->addAlert(ILARIA_ApplicationView::ALERT_TYPE_DANGER, "The episode \"" . $title . "\" was not inserted");
            }
        }
    ...
    public function action_update($request)
    {
        ...
        if ($request->existPostArg("f_" . self::FORM_NAME . "_dataid"))
        {
            ...
            // Update values
            if ($model->update($id, $title, $year, $episodeNumber, $seasonNumber) == 0)
            {
                // Load other controller, launch action and gather back view
                $controller = ILARIA_CoreLoader::getInstance()->loadController("production");
                $view = $controller->action_details(new ILARIA_CoreRequest(ILARIA_ConfigurationGlobal::buildRequestChain("production", "details", array('id' => $id))));
                $view->addAlert(ILARIA_ApplicationView::ALERT_TYPE_SUCCESS, "The episode \"" . $title . "\" was successfully updated");
                return $view;
            }
            else
            {
                // Register error
                $view->addAlert(ILARIA_ApplicationView::ALERT_TYPE_DANGER, "The episode \"" . $title . "\" was not updated");
            }
        }
    ...
    public function action_delete($request)
    {
        ...
        // Gather infos
        $infos = $model->getEpisodeInfos($id);
        ...
        if ($request->existGetArg("confirm"))
        {
            ...
            if ($model->delete($id) == 0)
            {
                // Load other controller, launch action and gather back view
                $controller = ILARIA_CoreLoader::getInstance()->loadController("production");
                $view = $controller->action_details(new ILARIA_CoreRequest(ILARIA_ConfigurationGlobal::buildRequestChain("production", "details", array('id' => $infos['serie_id']))));
                $view->addAlert(ILARIA_ApplicationView::ALERT_TYPE_SUCCESS, "The episode \"" . $infos['title'] . "\" was successfully deleted");
            }
            else
            {
                // Load other controller, launch action and gather back view
                $controller = ILARIA_CoreLoader::getInstance()->loadController("production");
                $view = $controller->action_details(new ILARIA_CoreRequest(ILARIA_ConfigurationGlobal::buildRequestChain("production", "details", array('id' => $id))));
                $view->addAlert(ILARIA_ApplicationView::ALERT_TYPE_DANGER, "The episode \"" . $infos['title'] . "\" failed to be deleted");
            }
        ...
        {
            // Create view
            $view = "<p>The episode \\\"" . $infos['title'] . "\\\" will be deleted. Are you sure ?</p>";
            ...
            return ILARIA_ApplicationAsynchronous::buildModalAjaxResponse(array(
                ...
                ILARIA_ApplicationAsynchronous::MODAL_BUTTONS => array(
                    ...
                    array(
                        ...
                        ILARIA_ApplicationAsynchronous::MODAL_BUTTON_LINK => ILARIA_ConfigurationGlobal::buildRequestChain("episode", "delete", array(
                            'id' => $id,
                            'confirm' => 'confirm',
                        )),
                    ),
