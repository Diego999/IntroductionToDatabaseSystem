...
class RoleController extends ILARIA_ApplicationController
{
    ...
    public function action_insert($request)
    {
        ...
        if ($request->existPostArg("f_" . self::FORM_NAME . "_dataid"))
        {
            ...
            if ($model->insert($name) == 0)
            {
                ...
                $view->addAlert(ILARIA_ApplicationView::ALERT_TYPE_SUCCESS, "The role \"" . $name . "\" was successfully inserted");
                return $view;
            }
            else
            {
                // Register error
                $view->addAlert(ILARIA_ApplicationView::ALERT_TYPE_DANGER, "The role \"" . $name . "\" was not inserted");
            }
        }
    ...
    public function action_update($request)
    {
        ...
        if ($request->existPostArg("f_" . self::FORM_NAME . "_dataid"))
        {
            ...
            if ($model->update($id, $name) == 0)
            {
                ...
                $controller = ILARIA_CoreLoader::getInstance()->loadController("directaccess");
                $view = $controller->action_miscellaneous(NULL);
                $view->addAlert(ILARIA_ApplicationView::ALERT_TYPE_SUCCESS, "The role \"" . $name . "\" was successfully updated");
                return $view;
            }
            else
            {
                // Register error
                $view->addAlert(ILARIA_ApplicationView::ALERT_TYPE_DANGER, "The role \"" . $name . "\" was not updated");
            }
        }
    ...
    public function action_delete($request)
    {
        ...
        if ($request->existGetArg("confirm"))
        {
            ...
            {
                $view->addAlert(ILARIA_ApplicationView::ALERT_TYPE_SUCCESS, "The role \"" . $infos['name'] . "\" was successfully deleted");
            }
            else
            {
                $view->addAlert(ILARIA_ApplicationView::ALERT_TYPE_DANGER, "The role \"" . $infos['name'] . "\" failed to be deleted");
            }
        ...
        {
            // Create view
            $view = "<p>The role \\\"" . $infos['name'] . "\\\" will be deleted. Are you sure ?</p>";
            ...
            return ILARIA_ApplicationAsynchronous::buildModalAjaxResponse(array(
                ...
                ILARIA_ApplicationAsynchronous::MODAL_BUTTONS => array(
                    ...
                    array(
                        ...
                        ILARIA_ApplicationAsynchronous::MODAL_BUTTON_LINK => ILARIA_ConfigurationGlobal::buildRequestChain("role", "delete", array(
                            'id' => $id,
                            'confirm' => 'confirm',
                        )),
                    ),
