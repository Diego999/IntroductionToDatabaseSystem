...
class TypeController extends ILARIA_ApplicationController
{
    ...
    public function action_insert($request)
    {
        ...
        if ($request->existPostArg("f_" . self::FORM_NAME . "_dataid"))
        {
            ...
            {
                // Register error
                $view->addAlert(ILARIA_ApplicationView::ALERT_TYPE_DANGER, "The type \"" . $name . "\" was not inserted");
            }
        }
    ...
    public function action_update($request)
    {
        ...
        if ($request->existPostArg("f_" . self::FORM_NAME . "_dataid"))
        {
            ...
            if ($model->update($id, $name) == 0)
            {
                ...
                $controller = ILARIA_CoreLoader::getInstance()->loadController("directaccess");
                $view = $controller->action_miscellaneous(NULL);
                $view->addAlert(ILARIA_ApplicationView::ALERT_TYPE_SUCCESS, "The type \"" . $name . "\" was successfully updated");
                return $view;
            }
            else
            {
                // Register error
                $view->addAlert(ILARIA_ApplicationView::ALERT_TYPE_DANGER, "The type \"" . $name . "\" was not updated");
            }
        }
    ...
    public function action_delete($request)
    {
        ...
        if ($request->existGetArg("confirm"))
        {
            ...
            {
                $view->addAlert(ILARIA_ApplicationView::ALERT_TYPE_SUCCESS, "The type \"" . $infos['name'] . "\" was successfully deleted");
            }
            else
            {
        ...
        {
            // Create view
            $view = "<p>The type \\\"" . $infos['name'] . "\\\" will be deleted. Are you sure ?</p>";
            ...
            return ILARIA_ApplicationAsynchronous::buildModalAjaxResponse(array(
                ...
                ILARIA_ApplicationAsynchronous::MODAL_BUTTONS => array(
                    ...
                    array(
                        ...
                        ILARIA_ApplicationAsynchronous::MODAL_BUTTON_LINK => ILARIA_ConfigurationGlobal::buildRequestChain("type", "delete", array(
                            'id' => $id,
                            'confirm' => 'confirm',
                        )),
                    ),
