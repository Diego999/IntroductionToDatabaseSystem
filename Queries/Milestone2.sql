...
FROM `production` P INNER JOIN `singleproduction` S ON P.id = S.id 
WHERE S.kind_id IN (SELECT K.id FROM `kind` K WHERE K.name IN ("tv movie","video movie","movie"))
GROUP BY P.year;
...
FROM(
	...
	FROM `casting` C INNER JOIN `role` R ON C.role_id = R.id
    WHERE R.name = "actor"
...
FROM 
	...
	AND C.production_id NOT IN (
								SELECT S.id	FROM `singleproduction` S INNER JOIN `kind` K ON S.kind_id = K.id WHERE K.name = "movie"
                                )) C1 
	,
	(SELECT C.person_id, C.production_id
	FROM `casting` C INNER JOIN `role` R ON C.role_id = R.id 
...
FROM (
    ...
    ORDER BY number DESC
    LIMIT 0,3
    ) T INNER JOIN `character` Ch
ON T.character_id = Ch.id;
